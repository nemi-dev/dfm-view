...
const initialState: SkillInputInit = {
  cases: [
    { name: "평타",value: 381, fixed: 96, useSkillInc: false,  atype: "Magic" },
    { name: "평타 막타",value: 505, fixed: 128, useSkillInc: false,  atype: "Magic" },
    { name: "프라이드 악셀", value: 3041, fixed: 3041, useSkillInc: true, atype: "Magic" },
    { name: "슬로스 바디", value: 8853, fixed: 8853, useSkillInc: true, atype: "Magic" },
    { name: "스커드 더 라스", value: 14106, fixed: 14107, useSkillInc: true, atype: "Magic" },
    { name: "정화의 꽃",value: 14322, fixed: 14322, useSkillInc: true,  atype: "Magic" },
  ]
}
export const skillInputSlice = createSlice({
  ...
  reducers: {
    ...
    SetSkillAtype: (s, { payload: [index, val] }: PayloadAction<[number, "Physc" | "Magic"]>) => {
      s.cases[index].atype = val
    },
    AddSkillCase: (s) => {
      s.cases.push({ name: `스킬 ${s.cases.length}`, value: 1000, fixed: 1000, useSkillInc: true, atype: s.cases.at(-1).atype })
    },
